...
// border-muted (not border-line) — border-line is ~1.5:1 against the canvas,
// which fails WCAG 1.4.11's 3:1 minimum for UI component boundaries and renders
// as an invisible outline; border-muted clears it.
const fieldClass =
  'w-full rounded-card border border-muted bg-surface px-3.5 py-2.5 text-base text-ink placeholder:text-muted transition-colors focus:border-accent'
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="mt-6 max-w-lg space-y-4">
      ...
      <button
        type="submit"
        disabled={submitting}
        className="inline-flex min-h-11 w-full items-center justify-center rounded-card bg-accent px-5 text-sm font-semibold text-on-accent transition-colors hover:bg-accent-hover disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
      >
